...
**Deployment produkcyjny:** Cloudflare Pages
`https://barcode-generator.daytodayapps.com`
Techniczny host Cloudflare Pages jest przekierowany 301 do domeny canonical.
...
- [x] Techniczny host Cloudflare Pages przekierowany 301 do domeny canonical.
...
- ✅ **Smoke testy konta i druku** — `tests/m25-account.spec.js` (16/16): noindex+canonical `daytodayapps.com` na `konto.html`, `moje-kody.html`, `szablony.html`, `drukarki.html`, `wydruk.html`, `historia-wydrukow.html`; struktura shell (nav + nag. + form) bez logowania. Dopełnia smoke per-language z `smoke.spec.js`.
...
- ✅ **sitemap.xml** — `lastmod` zaktualizowany na bieżącą datę dla wszystkich 22 wpisów (homepage + decoder × 11 lokali). Strony konta/builder/historii nadal poza sitemap (noindex,follow + canonical `daytodayapps.com`, świadoma decyzja — to obszar prywatny użytkownika).
...
- ✅ **`drukarki.html`** (NEW, EN-only, `noindex,follow`, canonical `daytodayapps.com`) — strona profili drukarek. Toolbar z licznikiem `{n}/{max}` i „New printer profile". Inline form: name, base preset (optgroup po vendor + "Custom"), printer type, page width/height, label width/height, cols/rows, margins (top/left) + gaps (x/y) w fieldset „Margins & gaps", calibration fieldset (DPI, offset X/Y ±20, bar width correction 0.5–1.5), checkbox „Set as default". Wybór presetu auto-wypełnia pola layoutu. Lista wykorzystuje `<template id="row-template">` z `.template-row`/`.template-summary`/`.template-meta`/`.badge-default` z Phase 3. Auth-gate przez `getSession()`; reload na `SIGNED_IN`/`SIGNED_OUT`.
...
- ✅ **`supabase-client.js`** — lazy ESM singleton. SDK jest dołączany do produkcyjnego buildu jako lokalny `/vendor/supabase.min.js`, więc konto nie zależy od dostępności zewnętrznego CDN. Eksportuje `getSupabase()`, `isSupabaseAvailable()`, `getSession()`, `onAuthStateChange()`. Storage key `bg.auth`, `persistSession: true`, `autoRefreshToken: true`. Formularz logowania renderuje się przed inicjalizacją SDK, a brak usługi daje czytelny komunikat zamiast pustego ekranu.
...
- **Produkcja:** https://barcode-generator.daytodayapps.com
