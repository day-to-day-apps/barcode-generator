 ...
 *
 * Depends on the global `JsBarcode` for linear symbols. QR markup can be
 * provided by the generator so preview and print use the exact same symbol.
 */
(function () {
    'use strict';
     ...
     * @param {string}  [opts.svgMarkup]       - Pre-rendered trusted SVG markup, used for QR labels.
    ...
    function createLabelHTML(opts) {
        ...
        const t               = opts.t || {};
        const svgMarkup       = typeof opts.svgMarkup === 'string' && /^<svg[\s>]/i.test(opts.svgMarkup.trim())
            ? opts.svgMarkup.trim()
            : '';

        const bcHeight  = Math.round((heightMM * bcHeightPercent) / 100);

        let svgHTML = svgMarkup;
        if (!svgHTML) {
